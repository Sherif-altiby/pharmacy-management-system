...
const Menu = ({ closeMenu, menuBlock }: { closeMenu: () => void; menuBlock: boolean }) => {
  return (
    <>
      <div
        className={` ${
          menuBlock ? "absolute top-16 right-0 left-0 z-30 bg-overlay bottom-0 " : ""
        } `}
        onClick={closeMenu}
      ></div>
      <div
        className={`fixed top-0 xl:left-[0] bg-white w-[220px] bottom-0 text-white ${
          menuBlock ? "left-0" : "left-[-220px]"
        } duration-300 z-50 dark:bg-black `}
      >
        <div>
          ...
          {items.map((item) => (
            <Link href={`/${item.link}`} key={item.title}>
              <div onClick={closeMenu} className="pl-3 pr-6 block py-2 cursor-pointer">
                <div
                  className="text-btn-color flex gap-2 rounded-md duration-200 justify-end hover:bg-page-color p-2 dark:text-white dark:hover:text-black"
                  data-aos="fade-right"
